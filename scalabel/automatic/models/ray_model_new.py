...
class RayModel(object):
    def __init__(self, cfg, logger):
        ...
        self.redis = redis.Redis(
            host=RedisConsts.REDIS_HOST, port=RedisConsts.REDIS_PORT
        )
        # self.logger.info(time.monotonic() - start_time)
    ...
    #     with Pool() as pool:
    #         urls = [item["urls"]["-1"] for item in item_list]
    #         image_list = pool.starmap_async(
    #             self.url_to_img, [(url, self.aug) for url in urls]
    #         )
    #         assert image_list == [
    #             self.url_to_img(url, self.aug) for url in urls
    #         ], "Image list corrupted"
    ...
    async def __call__(self, inputs, request_data, request_type):
        ...
        if self.cfg.TASK_TYPE == "box2d":
            if request_type == QueryConsts.QUERY_TYPES["inference"]:
                ...
                model_response_channel = (
                    self.model_response_channel % request_data["name"]
                )
                ...
                self.redis.publish(
                    model_response_channel,
                    json.dumps([pred_boxes, item_indices, action_packet_id]),
                )
            ...
            if request_type == QueryConsts.QUERY_TYPES["inference"]:
                ...
                boxes3d = [
                    box for box in boxes3d if np.linalg.norm(box[3:6]) > 4
                ]

                # Filter overlapping boxes
                boxes3d.sort(
                    key=lambda box: (np.linalg.norm(box[3:6]), box[3])
                )
                if len(boxes3d) > 1:
                ...
                self.redis.publish(
                    model_response_channel,
                    json.dumps([result, item_indices, action_packet_id]),
                )
        else:
            if request_type == QueryConsts.QUERY_TYPES["inference"]:
                ...
                self.redis.publish(
                    model_response_channel,
                    json.dumps([results, item_indices, action_packet_id]),
                )

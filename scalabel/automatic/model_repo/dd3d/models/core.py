...
from .postprocessing import nuscenes_sample_aggregate
...
class DD3D(nn.Module):
    def __init__(self, cfg):
        ...
        self.in_features = cfg.DD3D.IN_FEATURES or list(
            backbone_output_shape.keys()
        )
        ...
        self.prepare_targets = DD3DTargetPreparer(
            cfg, self.backbone_output_shape
        )
    ...
    def forward(self, batched_inputs):
        ...
        if "depth" in batched_inputs[0]:
            gt_dense_depth = [
                x["depth"].to(self.device) for x in batched_inputs
            ]
        ...
        inv_intrinsics = (
            images.intrinsics.inverse()
            if images.intrinsics is not None
            else None
        )
        ...
        if self.training:
            ...
            pred_instances = [
                Instances.cat(instances) for instances in pred_instances
            ]
            ...
            if not self.only_box2d and self.do_bev_nms:
                # Bird-eye-view NMS.
                dummy_group_idxs = {
                    i: [i] for i, _ in enumerate(pred_instances)
                }

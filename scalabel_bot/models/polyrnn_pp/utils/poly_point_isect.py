...
# only slight modification were made to make it works on python 2.7
...
class Event:
    ...
    def __init__(self, type, point, segment, slope):
        assert isinstance(point, tuple),"no tuple instaces"
        self.type = type
        self.point = point
...
def isect_polygon__naive_check(points)  :
    """
    Brute force O(n2) version of ``isect_polygon`` for test validation.
    """
    # isect = []

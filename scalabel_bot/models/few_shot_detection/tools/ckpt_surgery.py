...
def ckpt_surgery(args):
    ...
    def surgery(param_name, is_weight, tar_size, ckpt, ckpt2=None):
        ...
        if args.coco or args.lvis:
            for i, c in enumerate(args.BASE_CLASSES):
                idx = i if args.coco else c
                if "cls_score" in param_name:
                    new_weight[args.IDMAP[c]] = pretrained_weight[idx]
                else:
                    new_weight[
                        args.IDMAP[c] * 4 : (args.IDMAP[c] + 1) * 4
...
def combine_ckpts(args):
    ...
    def surgery(param_name, is_weight, tar_size, ckpt, ckpt2=None):
        ...
        if args.coco or args.lvis:
            for i, c in enumerate(args.NOVEL_CLASSES):
                if "cls_score" in param_name:
                    new_weight[args.IDMAP[c]] = ckpt2_weight[i]
                else:
                    new_weight[
                        args.IDMAP[c] * 4 : (args.IDMAP[c] + 1) * 4
...
def surgery_loop(args, surgery):
    ...
    # Surgery
    tar_sizes = [args.TAR_SIZE + 1, args.TAR_SIZE * 4]

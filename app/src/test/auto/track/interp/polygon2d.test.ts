...
import { LabelType, PathPoint2DType, PathPointType, SimplePathPoint2DType } from '../../../../js/functional/types'
...
test('2D polygon linear interpolation', () => {
  ...
  let shapes = state.task.items.map((item, index) =>
    labels[index].shapes.map((s) => item.shapes[s])) as PathPoint2DType[][]
  ...
  shapes = interp.interp(
    newLabel, newShape, labels, shapes) as PathPoint2DType[][]
  ...
  expect(shapes[2]).toMatchPoints2D(
    [{ x: 14, y: 19 }, { x: 14, y: 29 }, { x: 34, y: 19 }])
  expect(shapes[3]).toMatchPoints2D(
    [{ x: 18, y: 27 }, { x: 18, y: 37 }, { x: 38, y: 27 }])
  expect(shapes[5]).toMatchPoints2D(
    [{ x: 16, y: 23 }, { x: 16, y: 33 }, { x: 36, y: 23 }])
})

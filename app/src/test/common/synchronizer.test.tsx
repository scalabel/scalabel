...
describe('Test synchronizer functionality', () => {
  test('Test correct registration message gets sent', async () => {
    ...
    expect(selector.isStatusUnsaved(Session.getState())).toBe(true)
  })
  ...
  test('Test send-ack loop', async () => {
    ...
    expect(selector.isStatusSaved(Session.getState())).toBe(true)
  ...
  test('Test model prediction status', async () => {
    ...
    const ackPackets = sendAcks(sync)
    expect(selector.isStatusComputing(Session.getState())).toBe(true)
    ...
    expect(selector.isComputeDone(Session.getState())).toBe(false)

    sync.actionBroadcastHandler(
      packetToMessageBot(modelPackets[1]))
    expect(sync.numActionsPendingPrediction).toBe(0)
    expect(selector.isComputeDone(Session.getState())).toBe(true)
  })
  ...
  test('Test reconnection', async () => {
    ...
    expect(selector.isStatusReconnecting(Session.getState())).toBe(true)
    ...
    expect(selector.isStatusSaving(Session.getState())).toBe(true)
